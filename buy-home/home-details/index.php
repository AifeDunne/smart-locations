<html>
	...
	$pullData = "SELECT user_id, street_address, property_city, bedrooms, bathrooms, property_cost, property_size, property_available, property_lat, property_long, property_area, property_thumb, property_type, year_built, cooling, heating, fireplace, parking, lot_size, property_description, hasGallery FROM property_listing 
	LEFT JOIN property_details ON property_details.property_id = property_listing.property_id
	WHERE property_listing.property_id = ".$_GET['homeID'];
	$getProperty = $mysqli->query($pullData);
	$propertyDetails = $getProperty->fetch_array();
	$address = $propertyDetails['street_address'];
	$city = $propertyDetails['property_city'];
	$bedrooms = $propertyDetails['bedrooms'];
	$bathrooms = $propertyDetails['bathrooms'];
	$cost = $propertyDetails['property_cost'];
	$type = $propertyDetails['property_type'];
	$available = $propertyDetails['property_available'];
	$date = new DateTime($available);
	$available = $date->format('F d');
	$lat = $propertyDetails['property_lat'];
	$long = $propertyDetails['property_long'];
	$area = $propertyDetails['property_area'];
	$pic = $propertyDetails['property_thumb'];
	$size = $propertyDetails['property_size'];
	$year = $propertyDetails['year_built'];
	$cooling = $propertyDetails['cooling'];
	$heating = $propertyDetails['heating'];
	$fireplace = $propertyDetails['fireplace'];
	$parking = $propertyDetails['parking'];
	$lotsize = $propertyDetails['lot_size'];
	$description = $propertyDetails['property_description'];
	$hasGallery = $propertyDetails['hasGallery'];
	$person = $propertyDetails['user_id'];
  ...
  if ($pic !== "NULL") {  $firstRow.= "<div id='map-canvas'></div><div id='pictureBox'><img src='/buy-home/full-images/".$pic."' style='float:right; height:100%;'/></div></div>";  }
  else { $firstRow.= "<div id='map-canvas'></div><div id='pictureBox'><img src='/buy-home/full-images/None.jpg' style='float:right; height:100%;'/></div></div>"; }
  ...
  $fullString.= $thirdRow;
	...
	$boxWidth = "95vw";
	$box2 = "<div id='box2'>";
	$boxTitle = "<div id='box2T'>Home Tour</div>";
	...
	$imgPic = "Property".$_GET['homeID']."-".$nameArray[$mOne]."-Picture";
	$picCounter = 0;
	...
	$picString.= "<img id='rPic".$roomNum."-".$x."' class='vtElement roomPic".$roomNum." ".$pDisplay."' src='/buy-home/full-images/virtual-tour/".$fullPic."' style='float:left; width:24vw; max-height: 40vh; cursor:zoom-in;' />";
